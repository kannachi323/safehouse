...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error(error);
    return NextResponse.json({ error: 'Failed to prepare payload for google maps api' }, { status: 500 });
  }
  try {
    const googleApiUrl = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${userLatitude},${userLongitude}&destinations=${addresses}&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY_2}`;
